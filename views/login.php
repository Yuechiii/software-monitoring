...
<body class="relative min-h-screen flex flex-col items-center justify-center bg-(--accent)">
    <div class="flex flex-col-reverse lg:flex-row items-center font-roboto bg-white rounded-xl shadow-[8px_8px_7px_6px_rgba(17,76,86,0.4)] lg:m-10 p-10 lg:gap-10 transition-all duration-200 ease-in-out">
        <div>
            ...
            <form class="flex flex-col justify-center gap-y-3 font-roboto w-full" action="<?= $_SERVER['PHP_SELF'] ?>?f=validation" method="post">
                <input class="lg:text-[16px] text-xs rounded-lg border border-slate-300 placeholder-slate-500 placeholder:font-normal placeholder:normal-case font-bold text-(--base) bg-slate-200 pl-2 py-2 focus:outline-1 focus:outline-slate-400" type="name" name="_username" placeholder="Username">
